...
// ============================== CREATE Assignment ==============================
const createAssignment = async (payload: { moduleId: string; description: string; }) => {
...
const getAssignmentsIntoIntrutorCourses = async (instructorId: string, query: Record<string, unknown> = {}) => {
...
// ============================== UPDATE Assignment ==============================
const updateAssignment = async (id: string, payload: Partial<{ description: string; }>) => {
  const assignment = await prisma.assignment.findUnique({ where: { id } });
  if (!assignment) throw new CustomAppError(404, "Assignment not found for update");
...
const deleteAssignment = async (id: string) => {
  const assignment = await prisma.assignment.findUnique({ where: { id } });
  if (!assignment) throw new CustomAppError(404, "Assignment not found for deletion");
